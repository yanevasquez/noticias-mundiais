...
const apiKey = '474e537cae1947679d1a3d0935a68d01'
    ...
    fetch(url)
        ...
        .then(function(data) {
            let IhuU = data.articles
                .map(x => `<section class="noticiadoDiaTech"<h1>${link(x)}</h1>
                            ${imgNews(x)}<h2>${x.description}</h2>
                            <p>Fonte: ${x.source.name}</p><h3>${cverDate(x)}</h3>
                            </section>`)
                .join(" ")
    ...
    fetch(url)
        ...
        .then(function(data) {
            let bla = data.articles
                .map(x => `<section class="noticiaMTV"<h1>${link(x)}</h1>${imgNews(x)}<h2>${x.description}</h2>
                                <p>Fonte: ${x.source.name}</p><h3>${cverDate(x)}</h3>
                           </section>`)
                .join(" ")
    ...
    fetch(url)
        ...
        .then(function(data) {
            let vaiaparecerSim = data.articles
                .map(x => `<section class="noticiadoDiaScienc"<h1>${link(x)}</h1>
                                ${imgNews(x)}<h2>${x.description}</h2>
                                <p>Fonte: ${x.source.name}</p>
                                <h3>${cverDate(x)}</h3>
                            </section>`)
                .join(" ")
...
function trendBR() {
    ...
    fetch(url)
        ...
        .then(function(data) {
            let w = data.articles
                .map(x => `<section class="noticiaBr"<h1>${link(x)}</h1>
                                <p>Fonte: ${x.source.name}</p>
                                <h3>${converDate2(x)}</h3></section>`).join(" ")
            show_br.innerHTML = w
        })
}
    ...
    fetch(url)
        ...
        .then(function(data) {
            let tatu = data.articles
                .map(x => `<section class="noticiaCa"<h1>${link(x)}</h1>
                                    <p>Fonte: ${x.source.name}</p>
                                    <h3>${converDate2(x)}</h3>
                            </section>`)
                .join(" ")
...
function trendAR() {
    ...
    fetch(url)
        ...
        .then(function(data) {
            let k = data.articles
                .map(x => `<section class="noticiaAr"<h1>${link(x)}</h1>
                                    <p>Fonte: ${x.source.name}</p><h3>${converDate2(x)}</h3>
                           </section>`)
                .join(" ")
            show_ar.innerHTML = k
        })
